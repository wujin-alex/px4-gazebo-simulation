...
"""
TODO: [Alex]
使用jinja2模板渲染技术，使用.sdf.jinja模板生成.sdf文件，通常用于多机仿真（因为需要对每个飞机单独指定如udp端口等，不然使用同一个sdf生成的模型无法对各飞机进行区分）
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument('filename', help="file that the sdf file should be generated from")                      # [Alex], 所选模型文件夹下的.sdf.jinja文件
    parser.add_argument('env_dir')                                                                               # [Alex], 一般传入的路径为Tools/sitl_gazebo
    ...
    parser.add_argument('--output-file', help="sdf output file")                                                 # [Alex], 输出的.sdf文件，一般都放到/tmp文件夹下
    ...
    template = env.get_template(os.path.relpath(args.filename, args.env_dir))                                   #[Alex], 获取模板文件
    ...
    # TODO: [Alex], 定义字典保存渲染变量（来自本python脚本运行时传入的args）
    ...
    result = template.render(d)                                   #[Alex], 接收变量，对模板进行渲染（对模板文件中{{}}定义的对象使用传入的变量进行一一替换）
